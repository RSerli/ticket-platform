...
public class allTicketListController {
    ...
    @GetMapping
    public String mainView(@RequestParam(name = "keywordTicketName", required = false) String keywordTicketName,
            Model model) {
    ...
    public String refreshTec() {
        ...
        // per ogni tecnico faccio un sum-up dei ticket che ha e setto il suo stato di
        // disponiobilità
        ...
        for (user singlUser : allUsers) {
            ...
            List<ticket> toDoTicketsList = ticketRepo.findByUserAssignedAndActualStatus(singlUser,
                    statusRepo.findById(2).get());
            Integer numberToDoTickets = toDoTicketsList.size();

            List<ticket> toWorkTicketsList = ticketRepo.findByUserAssignedAndActualStatus(singlUser,
